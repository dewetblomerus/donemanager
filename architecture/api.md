...
```
POST https://{api_host}/v1/tags/{external_id}/scans
Authorization: Bearer <token>
```

- No request body.
- `{external_id}` is a UUIDv7 generated by the tag writer (NFC Tools Pro), not the backend.
- `{api_host}` is a dedicated API subdomain so the web UI can move or be rehosted without rewriting tags. *(Real domain still to be chosen.)*
