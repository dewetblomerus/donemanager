...
erDiagram
    ...
    tasks {
        ...
        time execute_window_start_time
        time execute_window_end_time
...
Users own their Pushover destinations directly. A user can belong to multiple households and take the same Pushover delivery setup with them. Households decide which users should be notified; each user's Pushover destinations decide where those notifications are delivered. Notifications are post-MVP — the first release is routine management only, with no Pushover send path.

## Notes

- Primary keys should use UUIDv7.
- Every `datetime` column above is a UTC instant stored as Ecto `utc_datetime_usec` (`timestamptz`). The `time` columns (`due_time`, `expiration_time`, `execute_window_start_time`, `execute_window_end_time`, `quiet_hours_start/end`) are wall-clock times of day, not instants, interpreted in `households.timezone`. See [decisions.md](decisions.md).
...
- `task_type` selects only the *next-`due_at` formula*; one shared reconcile loop drives all generation (see [scheduling.md](scheduling.md)). V1 values: `scheduled` (next due is the next wall-clock calendar slot from the cadence), `interval` (next due is the last completion plus the interval), and `timer` (an on-demand countdown started by an execute, such as a laundry timer — no auto-generation). Each type owns its own columns: `scheduled` uses `cadence_frequency`, `cadence_weekdays`, `due_time`, and a required `expiration_time`; `interval` uses `cadence_interval_minutes`; `timer` uses `timer_minutes`.
- Occurrence lifecycle is a single invariant for `scheduled` and `interval`: **a task has exactly one open occurrence; when it resolves, the loop creates the next.** An occurrence is *resolved* when `completed_at` is set **or** `expires_at < now` — completion is stored, expiry is derived. `scheduled` always has an `expires_at` (its slot resolves even if missed, so the next slot is created), while `interval` has `expires_at = null` and resolves only on completion (it stays open and grows more overdue — "3 days overdue" — until done, then rolls forward). The only per-type code is the next-`due_at` formula above; the loop, completion path, reminders, and idempotency are shared. There is no rolling-horizon pre-generation; future occurrences are computed for display, not stored.
...
- A task has a single `due_time` — one slot per cadence period. **A chore that happens several times a day is modelled as several tasks**, e.g. "Dog breakfast" and "Dog dinner", not one task with two times. This is deliberate: each occurrence then carries task-specific wording so a person knows at a glance whether *this* meal happened, and it composes with multi-task links — one tag by the food bowl drives both tasks, and the per-task `execute_window_*` routes a 7am tap to breakfast and a 6pm tap to dinner.
...
- `tasks.reminder_interval_minutes` (nullable, post-MVP) controls re-notification of an overdue occurrence: the backend keeps sending reminders at this cadence until the occurrence is completed, gated on the last `reminder` `notification_deliveries` row for the occurrence (the de-dup the dropped `task_events` table used to provide). Null means a single reminder.
...
- `GET /occurrences/{id}/execute` is where the side effect lives: it branches on `task_type` (`attempt_completion` or `toggle_timer`) and marks the occurrence done, then renders the occurrence page. It is a `GET` (an NFC tap is a navigation, and the core "tap → done" path must be frictionless) and is safe to reload because it is **occurrence-idempotent** — the id pins one specific occurrence, so a second hit finds it already resolved and just shows it. Re-tapping the *tag* tomorrow is a different occurrence and is correct to complete. `attempt_completion` only completes if not already resolved.
...
- `link_tasks` is a plain join of one `links` row to a task — `(household_id, link_id, task_id)`. A link may drive multiple tasks, and a task may be reachable from multiple links. To stop a link driving a task, delete the row. What an execute *does* is derived from the linked task's `task_type` at execute time, not stored on the binding, so editing a task's type can never leave a stale binding behind. The two V1 execute behaviors are `attempt_completion` (for `scheduled`/`interval` tasks) and `toggle_timer` (for `timer` tasks).
- When a link drives more than one task, execute considers each linked task's current occurrence within its task-level execute window, ignores completed occurrences while any open occurrence remains, sorts the open occurrences by `due_at`, and acts on the first due occurrence. This lets one physical tag cover related routines while staying deterministic. An execute that matches no task window is treated like an unassigned link and points the user to the web UI.
...
- `toggle_timer` is state-dependent. If no timer occurrence is active, it creates a delayed occurrence (`due_at = now + tasks.timer_minutes`). If a timer is already active, it cancels (deletes) that occurrence.
...
- `tasks.execute_window_start_time` and `tasks.execute_window_end_time` are nullable as a pair. Both null means a link execute may act on the task all day. When present, the window is start-inclusive and end-exclusive in the household timezone, so `05:00` to `11:00` matches executes at or after 05:00 and before 11:00. If the end is earlier than the start, the window crosses midnight. The window is a routing guardrail for multi-task links — separate from `tasks.expiration_time`, which controls how long an occurrence stays relevant for reminders and status. For a missed routine, the execute window may close before the occurrence expires so later taps route to the next routine, while completing the missed occurrence requires a deliberate web action.

## Retention

Old task history is deleted from `task_occurrences`. `notification_deliveries` is scoped to a `task_occurrence`; its foreign key should use `ON DELETE CASCADE`, so deleting an old occurrence also deletes its delivery records. This keeps retention simple: the app can delete occurrences older than the configured retention window without leaving orphaned delivery attempts.
