...
- `attempt_completion` is state-dependent. If the current occurrence is incomplete and unexpired, it sets `completed_at`/`completed_by_id`. If it was already completed, it does not undo the task; the user simply lands on the occurrence showing it is already done. If it is incomplete but expired (`expires_at < now`), it leaves it uncompleted and shows the `expired` state.
