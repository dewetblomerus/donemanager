...
erDiagram
    ...
    tasks {
        ...
        time scan_window_start_time
        time scan_window_end_time
...
- Every `datetime` column above is a UTC instant stored as Ecto `utc_datetime_usec` (`timestamptz`). The `time` columns (`due_time`, `expiration_time`, `scan_window_start_time`, `scan_window_end_time`, `quiet_hours_start/end`) are wall-clock times of day, not instants, interpreted in `households.timezone`. See [decisions.md](decisions.md).
...
- `users.quiet_hours_start` and `quiet_hours_end` are per-user times (interpreted in the household timezone). They are per-user from V1 because household members keep different sleeping hours. They shape *how* a notification is delivered, not a hard on/off: the Pushover sender reads the recipient's quiet hours and picks message priority (silent low priority during quiet hours, normal otherwise). Exactly which notifications send during quiet hours — confirmations always, reminders generally not, time-sensitive reminders maybe silently — is an open question (see [decisions.md](decisions.md)); the column shape does not change with the answer. The household decides who is notified; each user's quiet hours shape how; their Pushover destinations decide where.
...
- `tasks.reminder_interval_minutes` (nullable) controls re-notification of an overdue occurrence: the backend keeps sending reminders at this cadence — each recorded as a `reminder_sent` `task_event` — until the occurrence is completed. Whether reminders fire during a recipient's quiet hours is an open question (see [decisions.md](decisions.md)). Null means a single reminder.
...
- `tasks.scan_window_start_time` and `tasks.scan_window_end_time` are nullable as a pair. Both null means NFC scans may complete the task all day. When present, the scan window is start-inclusive and end-exclusive in the household timezone, so `05:00` to `11:00` matches scans at or after 05:00 and before 11:00. If `scan_window_end_time` is earlier than `scan_window_start_time`, the window crosses midnight.
- `automation_commands` links a tag to a task. A task may have one active tag binding, but a tag may be shared by multiple tasks. `label` is the UI/admin name for the link, such as `Dog food tag` or `Washer tag`. What a scan *does* is **derived from the linked task's `task_type` at scan time**, not stored on the link, so editing a task's type can never leave a stale command behind. The two V1 scan behaviors are `attempt_completion` (for `scheduled`/`interval` tasks) and `toggle_timer` (for `timer` tasks).
- When a scan matches more than one task for the tag's current task-level scan window, the backend considers each linked task's current occurrence, ignores completed occurrences while any open occurrence remains, sorts the open occurrences by `due_at`, and executes the command for the first due occurrence. This lets one physical tag cover related routines while preserving deterministic scan behavior. Scans that match no task scan window are treated like an unassigned tag.
- Task scan windows are intentionally separate from task expiration. A scan window controls when NFC autopilot may complete that task; `tasks.expiration_time` controls how long the occurrence remains relevant for reminders and status. For missed routines, the scan window may close before the occurrence expires so later scans route to the next routine, while completing the missed occurrence requires a deliberate web action.
- `attempt_completion` is state-dependent. If the current occurrence is incomplete, it produces a `completed` event. If it was already completed, it does not undo the task; it produces a `duplicate_completion_attempted` event and notifies the scanner.
- `toggle_timer` is state-dependent. If no timer occurrence is active, it creates a delayed occurrence (`due_at = now + tasks.timer_minutes`) and produces a `timer_started` event. If the timer is already active, it cancels that occurrence and produces a `timer_cancelled` event.
- `tasks.timer_minutes` (nullable, required for `timer` tasks) is the countdown length, e.g. `60` for a laundry move. It lives on the task — a user setting up "Laundry timer" expects to name the duration there — not on the link, so the tag binding stays a pure trigger.
