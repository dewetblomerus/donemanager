...
## Reminder notifications: per-user key, bounded-upsert deliveries, quiet hours soften

The first send path is live: reminders when a task is due/overdue (the loop's "Remind" step). Decisions made building it:

- **One Pushover key per user, not a `pushover_destinations` table.** The shipped foundation stores a single encrypted `users.pushover_user_key`. Multi-destination (label/device/enabled) is deferred until someone needs more than one device; `pushover_destinations` is dropped from the plan and `notification_deliveries` references `user_id` directly. See [database.md](database.md).
- **`notification_deliveries` is a current-state table, upserted — not an append log.** Exactly one row per `(task_occurrence_id, user_id, notification_type)`, updated in place each send (`last_sent_at`, `reminder_count`, `last_status`). This keeps it bounded by construction (so no partitioning is needed) and retained by `ON DELETE CASCADE` on the occurrence. The trade-off is no per-send audit history; current-state is all the reminder cadence needs. An append-log partitioned by day was considered and rejected as premature for this scale.
- **`notification_type` is `"reminder"` now; `"completed"` is reserved** for the later silent done-confirmation, which slots into the same table per recipient. The type is part of the unique key so a reminder's cadence and a one-shot completion don't collide on the same occurrence+user.
- **Recipients are all household members with a key set.** Household-curated notify lists remain undocumented/post-V1.
- **Quiet hours soften, they don't suppress.** A reminder still sends during a recipient's quiet hours, at Pushover priority `-1` (silent); normal priority otherwise. This matches the standing "how loud, not on/off" decision below. The time-sensitive overnight exceptions noted there are still open.
- **Timers are reminded though not generated by the loop.** The reconcile loop still skips timers for generation, but the Remind step scans all task types, so a timer past its `due_at` finally fires ("move the laundry"). This is the change that makes timer tasks actually do something.
...
## Migrations are additive-only; the prod DB is never reset

We are in alpha: prod holds real data — tasks for the author's own family and NFC tags written and linked to those tasks — that must be preserved. **Migrations are additive-only from here on, and the production database is not dropped or reset.** Never edit an already-applied migration in place; add a new one. Data fixes for rows written by older code are done with a forward migration or a careful, backed-up, narrowly-scoped SQL change against prod — never a reset.

(This supersedes the earlier one-time pre-launch reset, which edited migrations in place and dropped prod before each deploy. That was only ever safe while prod had no real data; it no longer does.)
