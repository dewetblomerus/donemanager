...
Each reminder carries a Pushover supplementary URL (`url_title: "Mark done"`) deep-linking to `GET /occurrences/{id}/execute` for **that** occurrence — the one the reminder is about, never a freshly-generated one. Tapping completes it in one tap and lands on the phone-formatted occurrence page. Because Pushover notifications persist in the tray, a stale reminder can be tapped long after the slot passed, so `execute` refuses to complete an **expired** occurrence (`expires_at < now`, not completed) and shows the `expired` state instead — otherwise a late tap on yesterday's missed slot would read "done" and risk double-doing today's.
