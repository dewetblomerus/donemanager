...
2. **Send reminders**: for each occurrence that is due/overdue and not completed, and for each notify recipient, send a Pushover notification when `now - last reminder_sent >= reminder_interval_minutes` and `now` is inside that user's waking hours. Record a `reminder_sent` `task_event` and a `notification_deliveries` row.
